...
static inline crc *new_bot(const crc *base_inj, const crc *base_proj) {
	crc *retc = (crc*)GC_MALLOC(sizeof(crc));
	retc->crckind = BOT;
	retc->g_proj = base_proj->g_proj;
	retc->g_inj = base_inj->g_inj;
	retc->p_proj = base_proj->p_proj;
	retc->crcdat.seq_tv.rid_proj = base_proj->crcdat.seq_tv.rid_proj;
	return retc;
}

// static inline crc *new_occur(const ty *tv, const crc *base_proj) {
// 	crc *retc = (crc*)GC_MALLOC(sizeof(crc));
// 	retc->crckind = OCCUR;
// 	retc->p_proj = base_proj->p_proj;
// 	retc->crcdat.seq_tv.ptr.tv = tv;
// 	retc->crcdat.seq_tv.rid_proj = base_proj->crcdat.seq_tv.rid_proj;
// }
...
// static inline crc* new_tv_proj_occur(const crc* base_proj, const crc *occur) {
// 	crc *c = (crc*)GC_MALLOC(sizeof(crc));
// 	c->crckind = TV_PROJ_OCCUR;
// 	c->p_proj = base_proj->p_proj;
//     c->crcdat.seq_tv.rid_proj = base_proj->crcdat.seq_tv.rid_proj;
// 	c->crcdat.seq_tv.ptr.s = occur;
// 	return c;
// }
...
crc* compose(crc *c1, crc *c2) {
	...
	switch(c1->crckind) {
		...
		case BOT: return c1; // ⊥;;s -> ⊥
		case SEQ_PROJ_BOT: return c1; // G?;⊥;;s -> G?;⊥
		case SEQ_PROJ: // G?;g;;s
		CASE_SEQ_PROJ: {
		...
		CASE_SEQ_PROJ_INJ: { // G?p;g;G!;;s
			...
			switch(c2->crckind) {
				case SEQ_PROJ_BOT: { // G?p;g;G!;;H?q;⊥r
					// printf("  c2.g_proj:%d, c2.c:%d\n", c2->g_proj, c2->crcdat.seq_tv.ptr.s->crckind);
					if (c1->g_inj == c2->g_proj) {   //G?p;g;G!;;G?q;⊥r -> G?p;⊥r
						crc *bot = c2->crcdat.seq_tv.ptr.s;
						return new_seq_proj_bot(c1, bot);
					} else {                         // G!p;g;G!;;H?q;⊥r -> G!p;bot^q (if G neq H)
						return new_seq_proj_bot(c1, new_bot(c1, c2));
					}
				}
				case SEQ_PROJ: 
				...
				case TV_PROJ: {										   	// G?p;g;G!;;X?q
					...
					if (occur_check(c1->crcdat.seq_tv.ptr.s, c2->crcdat.seq_tv.ptr.tv)) {
						blame(c2->crcdat.seq_tv.rid_proj, c2->p_proj);
						exit(1);
						// c2->g_proj = c1->g_inj;
						// return new_tv_proj_bot(c1, new_bot(c1, c2));
					}
		...
		case TV_INJ: {								    // X!p;;s
			...
			switch (c2->crckind) {     
				...
				CASE_TV_INJ_SEQ_PROJ: { // X!p;;G?q;g
					...
					if (occur_check(c2->crcdat.seq_tv.ptr.s, c1->crcdat.seq_tv.ptr.tv)) {
						blame(c2->crcdat.seq_tv.rid_proj, c2->p_proj);
						exit(1);
						// c1->g_inj = c2->g_proj;
						// return new_bot(c1, c2);
		...
		case TV_PROJ_INJ: {				// ?pX!q;;s
			...
			switch (c2->crckind) {   
				case SEQ_PROJ_BOT: { // ?pX!q;;G?p';⊥q' -> G?p;⊥q'
					dti(c2->g_proj, c1->crcdat.seq_tv.ptr.tv);
					normalize_tv_proj_inj(c1);
					return new_seq_proj_bot(c1, c2->crcdat.seq_tv.ptr.s);
				}   
				case SEQ_PROJ: 
				CASE_TV_PROJ_INJ_SEQ_PROJ: {  // ?pX!q;;G?r;g2 -> G?p;(g1;;g2)
					// printf("  c2.g_proj:%d, c2.c:%d\n", c2->g_proj, c2->crcdat.seq_tv.ptr.s->crckind);
					if (occur_check(c2->crcdat.seq_tv.ptr.s, c1->crcdat.seq_tv.ptr.tv)) {
						blame(c2->crcdat.seq_tv.rid_proj, c2->p_proj);
						exit(1);
