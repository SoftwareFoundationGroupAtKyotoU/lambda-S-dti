...
let array_init_int (n : int) (f : int -> int) : int array =
...
let array_init_int_array (n : int) (f : int -> int array) : int array array =
  let arr = Array.make n (f 0) in
  for i = 1 to n - 1 do
    arr.(i) <- f i
  done;
...
let payoff (current1 : int) (current2 : int) : float * float =
...
let array_init_automaton (n : int) (f : int -> automaton) : automaton array =
  let arr = Array.make n (f 0) in
  for i = 1 to n - 1 do
    arr.(i) <- f i
  done;
...
let choose_randomly (probabilities : float list) (speed : int) : int list =
...
let assert_ (v : float) (p : float -> bool) : float =
...
let payoff_p (x : float) : bool = x >=. 0.0 in

let rec evolve (p : automaton array * automaton array) (c : int) (s : int) (r : int) : float list =
  if c = 0 then []
  else
...
let rec print_floats (l : float list) : unit =
  match l with
  | [] -> ()
