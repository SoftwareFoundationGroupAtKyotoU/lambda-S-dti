...
module LS = struct
  ...
  let rec alpha_exp idenv = function
    ...
    | CAppExp (f, c) -> CAppExp (alpha_exp idenv f, c)
  ...
  let rec k_normalize_exp tvsenv = function
    ...
    | IfExp (f1, f2, f3) ->
      ...
      begin match f1 with
        ...
        | IConst _ | UConst | FunExp _ | FixExp _ -> raise @@ KNormal_bug "if-cond type should bool"
      end
    | FunExp (x, _, f) -> 
      let tent_var = genvar "_var" in
      let f = k_normalize_exp (Environment.add x [] tvsenv) f in
      KNorm.LetRecExp (tent_var, [], x, f, KNorm.Var tent_var)
    | FixExp _ -> raise @@ KNormal_bug "FixExp should appear in let"
    | AppExp (f1, f2) ->
      let f1 = k_normalize_exp tvsenv f1 in 
      let f2 = k_normalize_exp tvsenv f2 in
      insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm.AppExp (x, y)
    | CAppExp (f, c) -> 
      let f = k_normalize_exp tvsenv f in
      KNorm.CAppExp (f, c)
    | LetExp (x, tvs, f1, f2) -> 
  ...
  let k_normalize_program tvsenv = function
    ...
    | LetDecl (x, tvs, f) -> 
      begin match f with
        | FunExp (x', _, f) ->
          let f = k_normalize_exp (Environment.add x' [] tvsenv) f in
          KNorm.LetRecDecl (x, tvs, x', f), Environment.add x tvs tvsenv
        | FixExp (x', y, _, _, f) ->
          assert (x' = x);
          let f = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] tvsenv) f in
          KNorm.LetRecDecl (x, tvs, y, f), Environment.add x tvs tvsenv
        | _ as f ->
          let f = k_normalize_exp tvsenv f in 
...
module KNorm = struct
  ...
  let rec beta_exp idenv = function
    ...
    | CAppExp (f, c) -> CAppExp (beta_exp idenv f, c)
  ...
  let rec assoc_exp = function
    ...
    | CAppExp (f, c) -> CAppExp (assoc_exp f, c)
    | LetExp (x, f1, f2) ->
...
module LS1 = struct
  ...
  let rec alpha_exp idenv = function
    ...
    | LetExp (x, tvs, FixExp_alt ((x', y, u1, u2), k, (f1, f2)), f3) -> 
      assert (x = x');
      let newx = genvar x in
      let idenv = Environment.add x newx idenv in
      let newy = genvar y in
      ...
      LetExp (newx, tvs, FixExp_alt ((newx, newy, u1, u2), newk, (alpha_exp idenv' f1, alpha_exp idenv' f2)), alpha_exp idenv f3)
    | LetExp (x, tvs, f1, f2) -> 
      let newx = genvar x in
      LetExp (newx, tvs, alpha_exp idenv f1, alpha_exp (Environment.add x newx idenv) f2)
    | CoercionExp c -> CoercionExp c
    | AppExp_alt (f1, f2) -> AppExp_alt (alpha_exp idenv f1, alpha_exp idenv f2)
    | FunExp_alt ((x, u), k, (f1, f2)) -> 
      let newx = genvar x in
      let newk = genvar k in
  ...
  let alpha_program idenv = function
    ...
    | LetDecl (x, tvs, FixExp ((x', y, u1, u2), k, f)) ->
      ...
      let newy = genvar y in
      let newk = genvar k in
      LetDecl (newx, tvs, FixExp ((newx, newy, u1, u2), newk, alpha_exp (Environment.add y newy (Environment.add k newk idenv)) f)), idenv
    | LetDecl (x, tvs, FixExp_alt ((x', y, u1, u2), k, (f1, f2))) ->
      assert (x = x');
      let newx = genvar x in
      let idenv = Environment.add x newx idenv in
      let newy = genvar y in
      let newk = genvar k in
      let idenv' = Environment.add y newy (Environment.add k newk idenv) in
      LetDecl (newx, tvs, FixExp_alt ((newx, newy, u1, u2), newk, (alpha_exp idenv' f1, alpha_exp idenv' f2))), idenv
    | LetDecl (x, tvs, f) -> 
      let newx = genvar x in
      LetDecl (newx, tvs, alpha_exp idenv f), Environment.add x newx idenv

  let insert_let f (k: id -> KNorm1.exp) = match f with
    | KNorm1.Var x -> k x
    | _ as f -> 
      let x = genvar "_var" in
      let f' = k x in (* Var以外に自由な型変数への代入などは存在しないので、ここは[]で大丈夫 *)
      KNorm1.LetExp (x, f, f') (* todo:考える。とりあえず[] *)
      (*多分大丈夫、applicationにしか不要で、(fun x...) vなら既に代入済み、(fun (x:?)...) vならcast済み*)

  let rec k_normalize_exp tvsenv = function
    | Var (x, tas) -> 
      let tvs = Environment.find x tvsenv in 
      if List.length tas = 0 && List.length tvs = 0 then KNorm1.Var x
      else if List.length tas = List.length tvs then KNorm1.AppTy (x, tvs, tas)
      else raise @@ KNormal_bug "tas and tvs don't have same length"
    | IConst i -> KNorm1.IConst i
    | BConst b -> let i = if b then 1 else 0 in KNorm1.IConst i
    | UConst -> KNorm1.UConst
    | BinOp (op, f1, f2) as f -> begin match op with
      | Plus -> 
        let f1 = k_normalize_exp tvsenv f1 in
        let f2 = k_normalize_exp tvsenv f2 in
        insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.Add (x, y)
      | Minus ->
        let f1 = k_normalize_exp tvsenv f1 in
        let f2 = k_normalize_exp tvsenv f2 in
        insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.Sub (x, y)
      | Mult -> 
        let f1 = k_normalize_exp tvsenv f1 in
        let f2 = k_normalize_exp tvsenv f2 in
        insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.Mul (x, y)
      | Div -> 
        let f1 = k_normalize_exp tvsenv f1 in
        let f2 = k_normalize_exp tvsenv f2 in
        insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.Div (x, y)
      | Mod -> 
        let f1 = k_normalize_exp tvsenv f1 in
        let f2 = k_normalize_exp tvsenv f2 in
        insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.Mod (x, y)
      | _ -> k_normalize_exp tvsenv (IfExp (f, BConst true, BConst false))
      end
    | IfExp (f1, f2, f3) ->
      let f2' = k_normalize_exp tvsenv f2 in
      ...
      begin match f1 with
        ...
        | Var _ | BConst _ | IfExp _ | AppExp _ | AppExp_alt _ | LetExp _ | CAppExp _ as f ->
          let f = k_normalize_exp tvsenv f in 
          insert_let f @@ fun x -> insert_let (KNorm1.IConst 1) @@ fun y -> IfEqExp (x, y, f2', f3')
    ...
    | FunExp ((x, _), k, f) -> 
      ...
      KNorm1.LetRecExp (tent_var, [], (x, k), f, KNorm1.Var tent_var)
    | FixExp _ -> raise @@ KNormal_bug "FixExp should appear in let"
    | AppExp (f1, f2, f3) ->
      let f1 = k_normalize_exp tvsenv f1 in 
      let f2 = k_normalize_exp tvsenv f2 in
      let f3 = k_normalize_exp tvsenv f3 in
      insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> insert_let f3 @@ fun z -> KNorm1.AppExp (x, (y, z))
    | CAppExp (f1, f2) -> 
      let f1 = k_normalize_exp tvsenv f1 in
      let f2 = k_normalize_exp tvsenv f2 in 
      insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.CAppExp (x, y)
    | CSeqExp (f1, f2) ->
      let f1 = k_normalize_exp tvsenv f1 in
      let f2 = k_normalize_exp tvsenv f2 in 
      insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.CSeqExp (x, y)
    | LetExp (x, tvs, f1, f2) -> 
      begin match f1 with
        | FunExp ((y, _), k, f1) ->
          let f1 = k_normalize_exp (Environment.add y [] @@ Environment.add k [] tvsenv) f1 in
          let f2 = k_normalize_exp (Environment.add x tvs tvsenv) f2 in
          KNorm1.LetRecExp (x, tvs, (y, k), f1, f2)
        | FixExp ((x', y, _, _), k, f1) ->
          assert (x' = x);
          let f1 = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] @@ Environment.add k [] tvsenv) f1 in
          let f2 = k_normalize_exp (Environment.add x tvs tvsenv) f2 in
          KNorm1.LetRecExp (x, tvs, (y, k), f1, f2)
        | FunExp_alt ((y, _), k, (f1, f1')) ->
          let f1 = k_normalize_exp (Environment.add y [] tvsenv) f1 in
          let f1' = k_normalize_exp (Environment.add y [] @@ Environment.add k [] tvsenv) f1' in
          let f2 = k_normalize_exp (Environment.add x tvs tvsenv) f2 in
          KNorm1.LetRecExp_alt (x, tvs, (y, k), (f1, f1'), f2)
        | FixExp_alt ((x', y, _, _), k, (f1, f1')) ->
          assert (x' = x);
          let f1 = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] tvsenv) f1 in
          let f1' = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] @@ Environment.add k [] tvsenv) f1' in
          let f2 = k_normalize_exp (Environment.add x tvs tvsenv) f2 in
        ...
        | f ->
          let f1 = k_normalize_exp tvsenv f in
          let f2 = k_normalize_exp (Environment.add x tvs tvsenv) f2 in
          KNorm1.LetExp (x, f1, f2)
      end
    | CoercionExp c -> KNorm1.CoercionExp c
    | AppExp_alt (f1, f2) ->
      let f1 = k_normalize_exp tvsenv f1 in 
      let f2 = k_normalize_exp tvsenv f2 in
      insert_let f1 @@ fun x -> insert_let f2 @@ fun y -> KNorm1.AppExp_alt (x, y)
    | FunExp_alt ((x, _), k, (f1, f2)) ->
      let tent_var = genvar "_var" in
      ...
      KNorm1.LetRecExp_alt (tent_var, [], (x, k), (f1, f2), KNorm1.Var tent_var)
    | FixExp_alt _ -> raise @@ KNormal_bug "FixExp_alt should appear in let"

  let k_normalize_program tvsenv = function
    | Exp f -> let f = k_normalize_exp tvsenv f in KNorm1.Exp f, tvsenv
    | LetDecl (x, tvs, f) -> 
      begin match f with
        | FunExp ((x', _), k, f) ->
          let f = k_normalize_exp (Environment.add x' [] @@ Environment.add k [] tvsenv) f in
          KNorm1.LetRecDecl (x, tvs, (x', k), f), Environment.add x tvs tvsenv
        | FixExp ((x', y, _, _), k, f) ->
          assert (x' = x);
          let f = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] @@ Environment.add k [] tvsenv) f in
          KNorm1.LetRecDecl (x, tvs, (y, k), f), Environment.add x tvs tvsenv
        | FunExp_alt ((x', _), k, (f1, f2)) ->
          let f1 = k_normalize_exp (Environment.add x' [] tvsenv) f1 in
          let f2 = k_normalize_exp (Environment.add x' [] @@ Environment.add k [] tvsenv) f2 in
          KNorm1.LetRecDecl_alt (x, tvs, (x', k), (f1, f2)), Environment.add x tvs tvsenv
        | FixExp_alt ((x', y, _, _), k, (f1, f2)) ->
          assert (x' = x);
          let f1 = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] tvsenv) f1 in
          let f2 = k_normalize_exp (Environment.add y [] @@ Environment.add x' [] @@ Environment.add k [] tvsenv) f2 in
          KNorm1.LetRecDecl_alt (x, tvs, (y, k), (f1, f2)), Environment.add x tvs tvsenv
        | _ as f ->
          let f = k_normalize_exp tvsenv f in 
          KNorm1.LetDecl (x, f), Environment.add x tvs tvsenv
      end
end
...
module KNorm1 = struct
  ...
  let rec beta_exp idenv = function
    ...
    | AppExp_alt (x, y) -> AppExp_alt (find x idenv, find y idenv)
    | LetRecExp_alt (x, tvs, arg, (f1, f1'), f2) ->
      LetRecExp_alt (x, tvs, arg, (beta_exp idenv f1, beta_exp idenv f1'), beta_exp idenv f2)
  ...
  let beta_program idenv = function
    ...
    | LetRecDecl (x, tvs, arg, f) ->
      LetRecDecl (x, tvs, arg, beta_exp idenv f), idenv
    | LetRecDecl_alt (x, tvs, arg, (f1, f2)) ->
      LetRecDecl_alt (x, tvs, arg, (beta_exp idenv f1, beta_exp idenv f2)), idenv
  ...
  let rec assoc_exp = function
    ...
    | LetExp (x, f1, f2) ->
      let rec insert = function
        | LetExp (x', f3, f4) -> LetExp (x', f3, insert f4)
        | LetRecExp (x', tvs, arg, f3, f4) -> LetRecExp (x', tvs, arg, f3, insert f4)
        | LetRecExp_alt (x', tvs, arg, (f3, f3'), f4) -> LetRecExp_alt (x', tvs, arg, (f3, f3'), insert f4)
        | f1 -> LetExp (x, f1, assoc_exp f2)
      in insert (assoc_exp f1)
    | LetRecExp (x, tvs, arg, f1, f2) ->
      LetRecExp (x, tvs, arg, assoc_exp f1, assoc_exp f2)
    | LetRecExp_alt (x, tvs, arg, (f1, f1'), f2) ->
      LetRecExp_alt (x, tvs, arg, (assoc_exp f1, assoc_exp f1'), assoc_exp f2)
    | f -> f
...
let kNorm_funs ?(debug=false) (tvsenv, betaenv) f = 
  (* let f, alphaenv = LS.alpha_program alphaenv f in
  if debug then fprintf err_formatter "alpha: %a\n" Pp.LS.pp_program f; *)
  if debug then () else ();
  let f, tvsenv = LS.k_normalize_program tvsenv f in
  ...
  let rec iter betaenv f =
    ...
    if f = fassoc then f, (tvsenv, betaenv) else 
...
let kNorm1_funs ?(debug=false) (tvsenv, betaenv) f = 
  (* let f, alphaenv = LS1.alpha_program alphaenv f in *)
  (* if debug then fprintf err_formatter "alpha: %a\n" Pp.LS1.pp_program f; *)
  let f, tvsenv = LS1.k_normalize_program tvsenv f in
  if debug then fprintf err_formatter "k_normalize: %a\n" Pp.KNorm1.pp_program f;
  let rec iter betaenv f =
    let fbeta, betaenv = KNorm1.beta_program betaenv f in
    let fassoc = KNorm1.assoc_program fbeta in
    if f = fassoc then f, (tvsenv, betaenv) else 
      (if debug then fprintf err_formatter "beta: %a\n" Pp.KNorm1.pp_program fbeta;
      if debug then fprintf err_formatter "assoc: %a\n" Pp.KNorm1.pp_program fassoc;
      iter betaenv fassoc)
  in iter betaenv f

let kNorm1_funs_simpl ?(debug=false) (tvsenv, betaenv) f = 
  (* let f, alphaenv = LS1.alpha_program alphaenv f in *)
  (* if debug then fprintf err_formatter "alpha: %a\n" Pp.LS1.pp_program f; *)
  (* let f, tvsenv = LS1.k_normalize_program tvsenv f in
  if debug then fprintf err_formatter "k_normalize: %a\n" Pp.KNorm1.pp_program f; *)
