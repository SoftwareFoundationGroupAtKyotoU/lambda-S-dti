...
module Cls = struct
  ...
  let rec replace_var vx vy f = 
    ...
    | For (i, lo, hi, tag, f) -> For (i, replace lo, replace hi, tag, replace_var vx vy f)
    | While (f1, f2) -> While (replace_var vx vy f1, replace_var vx vy f2)
  ...
  let rec to_alt ids = function
    ...
    | For (i, lo, hi, tag, f) -> For (i, lo, hi, tag, to_alt ids f)
    | While (f1, f2) -> While (to_alt ids f1, to_alt ids f2)
