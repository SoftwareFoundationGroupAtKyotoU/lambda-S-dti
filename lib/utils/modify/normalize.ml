...
let rec make_static_proj_coercion ~monotonic u (r, p) = match normalize_type u with
  | TyInt | TyBool | TyUnit | TyFloat | TyString as g -> CSeq (CProj (tag_of_ty g, (r, p)), CId g)
...
and make_static_inj_coercion ~monotonic u (r, p) = match normalize_type u with
  | TyInt | TyBool | TyUnit | TyFloat | TyString as g -> CSeq (CId g, CInj (tag_of_ty g))
...
let rec make_static_projinj_coercion ~monotonic (r1, p1) u (r2, p2) = match u with
  | TyInt | TyBool | TyUnit | TyFloat | TyString as g -> CSeq (CProj (tag_of_ty g, (r1, p1)), CSeq (CId g, CInj (tag_of_ty g)))
  | TyFun (u1, u2) ->
