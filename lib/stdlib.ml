...
let pervasives ~config =
  ...
  let initial_compile_env =
    (Environment.empty, Environment.empty, Environment.empty, Environment.empty), V.empty, Environment.empty
  in
  let add_to_compile_env ((alphaenv, tvsenv, betaenv, opt_tvs_env), known, args) builtin = match builtin.c_backing with
    | CImpl cname ->
      (* No CImpl builtin uses its type variables at runtime (the one case that
         would need to -- monotonic array creation in array_init -- is instead
         handled as Array.make-based syntax sugar, never as a CImpl), so every
         one of them is registered here as using none of its type variables. *)
      (Environment.add builtin.name cname alphaenv,
       Environment.add cname (pick_tvs builtin.name) tvsenv,
       Environment.add cname cname betaenv,
       Environment.add cname TV.empty opt_tvs_env),
      V.add cname known, Environment.add cname ([], 0) args
    | CUnimplemented -> (alphaenv, tvsenv, betaenv, opt_tvs_env), known, args
  in
