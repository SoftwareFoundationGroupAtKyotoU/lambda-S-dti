# Phase H — Reference/Array実装・utils再構成・bench基盤の整理（2026-05-27〜2026-09-02）

[docs/history.md](../history.md) 本体から参照。一つ新しい話題は [docs/history.md](../history.md) 本体（最新の話題）へ、一つ古い話題は [Phase G](phase-g-pipeline-tuple.md) へ。

Reference（`ref`）と Array の実装が一つの coercion 設計パターン（read/write を分けた `CRef`/`CArray` と、型変数を単方向インスタンス化のみ許す monotonic 版 `CMRef`/`CMArray`）で揃った時期。並行して `lib/utils/` のファイル分割、二項演算子の表現統一、capp のショートサーキット最適化など、後続の性能最適化（[docs/history.md](../history.md) の最新の話題を参照）の土台になる整理が進んだ。末尾で bench がライブラリ化されコンパイル専用になる。
...
- **インタプリタ計測を bench から撤去**しコンパイル専用に。旧 `mem_json` の
  ...
  `test/test_mutate.ml` の差分カバレッジテストへ移した。
...
（この `bench/` ライブラリ構成は約2週間後の並列コンパイル化・`lib/bench/` への移行で再度大きく作り替えられる。詳細は [docs/history.md](../history.md) を参照）
...
これまで `README.md` の構文一覧が古く（pattern match/list/tuple/reference/array/loop が未記載）、`TODO.md` の内容は古い課題メモが中心だった。README に pattern matching（`match`/`function`、変数・リテラル・wildcard・list/tuple パターンとその入れ子）、list（`[e1; e2; ...]`, `::`）、tuple、reference（`ref`/`!`/`:=`）、array（`Array.make`/`.()`/`Array.length`）、loop（`for`/`while`、いずれも `fun`/`let rec` への脱糖）の構文と、対応する型構文（`list`/`tuple`/`ref`/`array`）を追記。合わせて `TODO.md` を `memo.md` にリネームし、古い内容を削除（現状の残作業は `docs/todo.md` に一元化されている）。

（この時点の `for`/`while` は「`fun`/`let rec` への脱糖のみで新規 AST ノードなし」という設計だったが、約1ヶ月後にネイティブ C の `for`/`while` 文を直接出力する専用 AST ノードへ置き換えられる。詳細は [docs/history.md](../history.md) を参照）
...
以前の CLAUDE.md には「injection 側のみシングルトン化済み、projection 側は blame 情報を持つため未着手」という TODO が書かれていたが、この commit で projection 側（tag 単体・tuple 双方）にも対応した。ただし「シングルトン化」ではなく「静的に判別可能なパターンを検出してインライン展開する」という別アプローチである点に注意。

（`toplevel_coerce_*` は約1ヶ月半後に `apply_coerce_*` へリネームされる。詳細は [docs/history.md](../history.md) を参照）
...
（この `sc_push`/`consume` による suspended-cast キューは、約1ヶ月半後に `coerce` へ `suspend` フラグを持たせる形へ発展し、deref/subst/get/put の非 suspend 経路で即時適用できるようになる。詳細は [docs/history.md](../history.md) を参照）
